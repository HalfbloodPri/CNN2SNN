...
'''
在前向计算的过程中，采用二值化的权重，大于等于0的权重为1，小于0的权重为-1；
'''
...
W_conv1_Bi = tf.where(W_conv1<0,-1,1)
h_conv1 = [None,None,None,None,None,None,None,None,None]
h_pool1 = [None,None,None,None,None,None,None,None,None]
for i in range(3):
    for j in range(3):
        h_conv1[i*3+j] = tf.nn.relu(conv2d(x_image[:,(8*i):(8*i+12),(8*j):(8*j+12),:],W_conv1_Bi))
...
W_conv2_Bi = tf.where(W_conv2<0,-1,1)
h_conv2 = [None,None,None,None]
h_pool2 = [None,None,None,None]
for i in range(2):
    for j in range(2):
        h_conv2[i*2+j] = tf.nn.relu(conv2d(h_pool1_merge[:,(4*i):(4*i+8),(4*j):(4*j+8),:],W_conv2_Bi))
